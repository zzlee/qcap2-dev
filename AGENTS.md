...
- Test in Docker (mounting local workspace, clean build dir): `docker run --rm --user $(id -u):$(id -g) -v $(pwd):/workspace qcap2-build-env sh -c "rm -rf build && mkdir -p build && cd build && cmake .. && make -j\$(nproc) && ctest --output-on-failure"`
